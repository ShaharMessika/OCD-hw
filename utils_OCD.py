...
batch=None, loss_fn=None,std=0,dopt=0):
    ...
    model.get_parameter(named_parameter+'.weight').data = r + std*w.squeeze().to('cuda')
    predicted_labels,h = model(batch['input'])
    #acc = print(predicted_labels.max(1)[1],1,h[0].max(1),2,batch['output'],3,batch['output'].long(),4)
    test_acc = (predicted_labels.max(1)[1] == batch['output']).sum().item()
    m = batch['output'].size(0)
    acc = test_acc
    ...
    del model
    return ldiffusion,loptimal,lbase, acc,m
...
def generalized_steps(named_parameter, numstep, x, model, bmodel, batch, loss_fn, std, padding, mat_shape, isnerf=0, **kwargs):
    with torch.no_grad():
        ...
        wdiff = xs[-1]
        ldiffusion,loptimal,lbase, acc, m = check_ps_wrapper(isnerf=isnerf,named_parameter=named_parameter,
            bmodel=bmodel, w=wdiff.squeeze(), batch=batch,
            loss_fn=loss_fn,std=std,dopt=dopt
            )
    return ldiffusion,loptimal,lbase,wdiff, acc, m
