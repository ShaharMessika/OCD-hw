...
def train(args, config, optimizer, optimizer_scale,
    ...
    for epoch in range(epochs):
        ...
        if ((epoch + 1) % n_checkpoint == 0) or (epoch + 1 == epochs):
            ...
            torch.save(ema_helper.state_dict(),checkpoint_path+f'model_checkpoint_epoch{epoch+200}_step{step}_data{args.datatype}.pt')
            torch.save(scale_model.state_dict(),checkpoint_path+f'scale_model_checkpoint_epoch{epoch+200}_loss{step}_data{args.datatype}.pt')
