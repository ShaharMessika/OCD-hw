...
parser.add_argument(
    '-pb', '--backbone_path', type=str, default = 'E:/Users/shaha/Documents/GitHub/convmixer-cifar10/weights/checkpoint_epoch324.pt',
    help='checkpoint path for backbone, default </base_models/checkpoint_tinynerf.pt>')
parser.add_argument(
    '-pc', '--config_path', type=str, default = './configs/train_convmixer.json',
...
parser.add_argument(
    '-pdtr', '--data_train_path', type=str, default = './data/cifar-10-batches-py',
    help='training data path, default <''/data/tiny_nerf_data.npz''>')
parser.add_argument(
    '-pdts', '--data_test_path', type=str, default = '/data/cifar-10-batches-py',
    help='test data path, default <''/data''>')
parser.add_argument(
    '-dt', '--datatype', type=str, default = 'ConvMaxSpread',
...
dmodel_original_weight = torch.squeeze(dmodel_original_weight)
...
print('*'*100)
ldiff,lopt,lbaseline,accuracy = 0,0,0,0
for idx, batch in enumerate(test_loader):
    ...
    ldiffusion, loptimal, lbase, wdiff, acc, m = generalized_steps(
    ...
    lbaseline += lbase
    accuracy += acc
    print(f"\rBaseline loss {lbaseline/(idx+1)}, Overfitted loss {lopt/(idx+1)}, Diffusion loss {ldiff/(idx+1)}, acc {accuracy/idx+1} ",end='')
